...
{    
	...
	std::cout << " ok im 1 " << filename1 <<  std::endl; 
		...
		cv::Mat color00;
		depth00 = depth.clone();
		resize(depth00, depth, Size(320, 240));
		color00 = color.clone();
		resize(color00, color, Size(320, 240));
			std::cout << " ok read " << color.rows << std::endl;
		/*for (int j = 0; j < 320; j++)
		for (int i = 0; i< 240; i++)
		{
		std::cout << " color " << (int)color.at<Vec4b>(i,j)[3] << std::endl;
		}*/

              //cv::imwrite("color.jpg",color);	
		
	        cv::namedWindow("depth_xtion",WINDOW_AUTOSIZE );
		cv::imshow("depth_xtion",depth);		
		cv::namedWindow("image_xtion",WINDOW_AUTOSIZE );
