...
__global__
void TrimapFromMaskKernel(Npp8u *alpha, int alpha_pitch, Npp8u *dt, int width, int height)
{

    Npp32u *alpha4 = (Npp32u *)alpha;
    Npp32u *dt4 = (Npp32u *)dt;
    int alpha4_pitch = alpha_pitch / 4;
    ...
    for (int i=threadIdx.y; i<32; i+=blockDim.y)
    {
        ...
        if (x< width && y < height)
        {
            ...
            if (dt4[y * alpha4_pitch + x0] == 0x0ffffffffu)
            {
                //int first_x = min(max(0, rect.x - x),4);
                //int last_x = min(max(0,x - (rect.x + rect.width - 4)),4);
                ...
                //alpha4[y * alpha4_pitch + x0] = (pattern << (first_x *8)) & (mask >> (last_x*8));
                alpha4[y * alpha4_pitch + x0] = 0x0ffffffffu;
            }
            else
            {
                alpha4[y * alpha4_pitch + x0] = 0;
...
cudaError_t TrimapFromMask(Npp8u *alpha, int alpha_pitch, Npp8u *dt, int width, int height)
{
    ...
    TrimapFromMaskKernel<<<grid, block>>>(alpha, alpha_pitch, dt, width, height);
