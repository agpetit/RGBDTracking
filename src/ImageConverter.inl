...
ImageConverter<DataTypes, DepthTypes>::ImageConverter()
    : Inherit()
    , depthImage(initData(&depthImage,DepthTypes(),"depthImage","depth map")) //input from dataexchange
    , image(initData(&image,ImageTypes(),"image","image")) //input from data exchange
    , useRealData(initData(&useRealData,true,"useRealData","Use real data")) // both must be active for image rescaling
    , useSensor(initData(&useSensor,false,"useSensor","Use the sensor"))    // same as above
    , sensorType(initData(&sensorType, 0,"sensorType","Type of the sensor")) // deprecated
    , niterations(initData(&niterations,1,"niterations","Number of iterations in the tracking process")) // how many time steps for each processing
