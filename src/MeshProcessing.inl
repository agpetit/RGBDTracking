...
#define CANNY_TH1 350
#define CANNY_TH2 10

    cv::Mat contour,dist,dist0,depthmapS;
    //cv::imwrite("depthmap.png", depthMap);
    cv::Canny( depthMap, contour, CANNY_TH1, CANNY_TH2, 3);
    ...
    cv::Mat contourpoints = cv::Mat(hght,wdth,CV_8UC3,cv::Scalar(255,255,255));
    for (int j = 0; j < wdth; j++) {
        for (int i = 0; i< hght; i++) {
...
void MeshProcessing<DataTypes>::handleEvent(sofa::core::objectmodel::Event *event)
{
    if (dynamic_cast<simulation::AnimateBeginEvent*>(event)) {
        ...
        if (!depthrend.empty() && useVisible.getValue() &&
            (int)this->getContext()->getTime()%niterations.getValue()!= 0
        ) {
            if(useContour.getValue()) {
        ...
        helper::AdvancedTimer::stepEnd("MeshProcessing") ;
    }
}
